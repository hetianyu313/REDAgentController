...
void UnlockKeyboard(){
	...
	if (!hDll){
		cout << "DLL获取失败! error: " << GetLastError() << endl;
		return;
	}
	...
	if (!EnableInputMessage){
		cout << "函数加载失败! error: " << GetLastError() << endl;
		FreeLibrary(hDll);
		return;
	}
...
typedef NTSTATUS (NTAPI *pNtCreateThreadEx)(
    ...
    POBJECT_ATTRIBUTES ObjectAttributes,
    HANDLE ProcessHandle,
    PVOID StartRoutine,       // 目标函数地址
    PVOID Argument,           // 传给 StartRoutine 的参数
    ULONG CreateFlags,
    SIZE_T ZeroBits,
    SIZE_T StackSize,
...
void ThreadG_Function() {
	...
	while (g_running) {
        if (g_threadG_active) {
	        HHOOK kbdHook = (HHOOK)SetWindowsHookEx(WH_KEYBOARD_LL, (HOOKPROC)HookProc, hModule, 0);
	        Sleep(25);
